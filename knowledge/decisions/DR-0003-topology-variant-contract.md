...
- **capability-index capability/swap-class auto-approve exposure.** The **central** `catalog/capability-index.yaml` carried a review exposure analogous to the one the `pr:triage` withhold closes for `catalog/topology-groups.yaml` (a single-file edit — adding an implementation, changing a `swap_class`, moving a status — asserts human judgment with no sound mechanical oracle), and it was pre-existing. Deliberately out of scope for #733; **closed by #752** for that central file with the narrowest mechanism — a single-file glob `^catalog/capability-index\.yaml$` added to the `pr:triage` governance-withhold set (`Taskfile.yml`), red-green-bound by a `test:pr-triage` case (PR#129 → `governance`). Not a broad `catalog/` withhold (that would over-withhold routine per-component-directory edits under `catalog/`). **Coverage is asymmetric with the #733 sibling, not identical.** The topology-groups withhold covers that contract's *whole* machine-readable surface: a per-component topology copy cannot exist — `schemas/compatibility.schema.json` is `additionalProperties: false` with no topology field (a per-component `topology_group` was explicitly rejected, §"Why a central file"), so the topology claim lives only in the withheld central file. This capability-index withhold does not have that property: `swap_class` **is** a schema-permitted per-component field (`provides[].capabilities[].swap_class`), duplicated across ~40 `sub-layers/**/compatibility.yaml` files — which `catalog/README.md` requires to match this index — and those per-component copies (plus their capability-id claims) remain a **separate, unwithheld** surface a component PR can still edit auto-approvably. `schemas/compatibility.schema.json` checks their shape but not their agreement with this index (no cross-file oracle). So #752 closes the *central-file* exposure; the per-component copy exposure is a genuine coverage residual (not merely a soundness boundary like the #733 sibling's) left for a future hardening pass — a cross-file index↔component `swap_class`/capability-id agreement gate, keyed on the real `provides[].capabilities[].swap_class` path — not #752.
...
- ~~File the `catalog/capability-index.yaml` capability/swap-class hardening follow-up issue (see the residual above).~~ Filed as #752 and **closed for the central file** by the `pr:triage` withhold-glob extension; the per-component `provides[].capabilities[].swap_class` coverage residual (see the residual above) remains open for a future cross-file agreement gate.
