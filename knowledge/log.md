...
- **Build-contract hardening (#737)**: `.claude/skills/build-catalog-component/CONVENTIONS.md` gained two new sections — `## Reading rendered artifacts` (a bounded-window reading discipline for generated artifacts over 500 lines: inventory via `^kind:`, then lens-bounded windows, with an un-reviewed report and a critical `not-read:` finding when the artifact is absent) and `## Render-grounded claims` (a claim about rendered output or chart behaviour is written only after being read from the render or the chart template, binding the builder's own work, every orchestrator-authored brief, and every render claim the orchestrator itself cites). `.claude/skills/build-catalog-component/SKILL.md` carries both disciplines into the five render-inspecting dispatch briefs — the Phase-3 evaluator, the Phase-4 fixer, the Phase-5 reviewers, the Phase-5 semantic-bearing-fix re-dispatch, and the Phase-6 step-2 evaluator re-dispatch — so no brief depends on the orchestrator remembering either rule. Re-verified `workflows/catalog-build-pipeline.md` and `decisions/DR-0001-specification-driven-component-build.md` (both list the edited `CONVENTIONS.md` in `sources:`): purely additive, no present-tense claim falsified; timestamps bumped.
