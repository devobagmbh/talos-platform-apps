...
| `task worktree:pr -- <N>` | isolated worktree on an EXISTING PR head branch (`.claude/worktrees/pr-<N>`, checked out at `origin/<head>`); idempotent, refuses a diverged local branch — the `pr-fix` isolation half |
| `task worktree:pr:remove -- <N>` | remove a PR-fix worktree (branch kept; refuses while changes are uncommitted) |
| `task pr:fix:facts -- <N>` | read-only review-response fact sheet for one PR as JSON (reviews + unresolved GraphQL review threads + head anchor + approvals-at-head) |
...
**Responding to a review (`pr-fix`)**: `/pr-fix <N>` is the third member of the PR family and the only one that writes code — it answers *what do the findings require, and what did I change*, for a PR **the running identity authored**. Four properties make it safe to run repeatedly: (1) **triage before fix** — every finding is reproduced against the PR head before it is applied and rejected with cited counter-evidence when it does not hold (the in-the-wild rejection rate for agentic review comments is 56.3%, and this repo has two standing false-positive classes of its own: faithfully-vendored upstream defaults and established repo conventions); (2) **deterministic admissibility** — `task pr:fix:facts` + the tested `task pr:fix:admit` classifier decide whether a fix may be built at all (`fork` / `not-author` / `dirty` / `no-findings` / `incomplete` all stop the run), so the security-relevant half is a red-green target, not skill prose; (3) **isolation** — every fix is built in `.claude/worktrees/pr-<N>` at `origin/<head>` via `task worktree:pr`, one commit per finding, each commit body carrying its verification; (4) **it ends before the push** — the report names the `approvals-at-risk` count (a push dismisses standing approvals under `dismiss_stale_reviews`), and the push, the thread replies and the merge stay operator-authorized. It never resolves a review thread to clear `required_conversation_resolution`, never rebases, and no unattended loop dispatches it. Spec: `.claude/skills/pr-fix/SKILL.md`.
