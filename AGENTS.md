...
- **Capability-first component contracts** ([talos-platform-docs ADR-0021](https://github.com/devobagmbh/talos-platform-docs/blob/main/adr/0021-capability-layer-model.md)) — in `compatibility.yaml`, each `provides[]` entry names its tool/chart (`name:`, mandatory) and *additively* lists the capabilities that tool implements under `provides[].capabilities[]` (each `{id, swap_class}`; every `id` must exist in `catalog/capability-index.yaml`). The capability id is the stable interface a consumer composes against; whether the tool behind it swaps without a consumer-visible break is governed by `swap_class` (only `drop-in` is invisible — `data-migration` / `rewrite-required` / `consumer-change` are not). Components with no matching capability legitimately carry `capabilities: []` (e.g. the `-crds` strict-B halves, apis-only components). Full shape: §Sub-layer conventions above.
