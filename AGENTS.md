...
**Catalog-Ship-Orchestrator**: Das Skill `ship-catalog-app` orchestriert den vollen plan→approve→build-Bogen EINER App in einer Session als dünne Schicht über die zwei Skills unten (dupliziert deren Logik nicht). Drei Invarianten: ein **verpflichtender menschlicher Plan-Freigabe-Gate** zwischen Plan und Build (headless → stop-after-plan); ein **mechanisch vorklassifizierter Merge-Gate mit dem Build-Skill als Backstop** — abhängige Komponenten brauchen den Merge ihrer Dependency, bevor der frische Worktree von `origin/main` sie sieht; ship's konservativer Pre-Check ist nur eine Dispatch-Spar-Optimierung vor diesem Backstop — einzige Garantie ist **kein falscher Build** (der autoritative Build-Check stoppt jede unzulässige Komponente); beide Fehlklassifikationen sind unschädlich (Über-Zulassung → verschwendeter Dispatch; false-stall bei extern nach dem Fetch gemergter Dependency → transient, vom Re-run gelöst); und **Re-run-Resumierbarkeit** aus dem Git-Status, kein Ship-State-File. Spec/DRY-Quelle: die SKILL.md (kein eigenes CONVENTIONS.md). Für Einzel-Komponenten ohne Plan genügt direkt `build-catalog-component`.
