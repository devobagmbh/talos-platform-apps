...
**Catalog-Ship-Orchestrator**: Das Skill `ship-catalog-app` orchestriert den vollen plan→approve→build-Bogen EINER App in einer Session als dünne Schicht über die zwei Skills unten (dupliziert deren Logik nicht). Es erzwingt einen **verpflichtenden menschlichen Plan-Freigabe-Gate** zwischen Plan und Build. Den **Merge-Gate klassifiziert ship mechanisch vor dem Dispatch, der Build-Skill ist der Backstop**: abhängige Komponenten brauchen den Merge ihrer Dependency, bevor ihr frischer Worktree von `origin/main` sie sieht. Ship prüft per billigem, konservativem Pre-Check, ob die plan-deklarierten `external_dependencies` einer Komponente alle nach `main` gemerged sind — wenn nicht, ist sie `awaiting-merge` und wird nicht attemptet (kein verschwendeter Build). Der Pre-Check ist strikt schwächer als der Build-Gate (`external_dependencies ⊆ external_dependencies ∪ requires:`), der Build-Skill-Check bleibt autoritativer Backstop für Build-Zeit-`requires:`; jeder Stop nach einem Attempt ist eindeutig `build-incomplete`. **Re-run-resumierbar** über den Git-Status (`done`/`in-flight`/pending) plus positivem Completion-Verify (`git ls-remote` + `gh pr list`) vor jedem Vorrücken — kein Ship-State-File, „done" wird nie inferiert. Spec/DRY-Quelle: die zwei Sub-Skills (kein eigenes CONVENTIONS.md). Für Einzel-Komponenten ohne Plan genügt direkt `build-catalog-component`.
