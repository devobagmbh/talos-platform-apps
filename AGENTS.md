...
| `task worktree:pr -- <N>` | isolated worktree on an EXISTING PR head branch (`.claude/worktrees/pr-<N>`, checked out at `origin/<head>` with `core.symlinks=false` + `GIT_LFS_SKIP_SMUDGE=1`); idempotent but re-verifies a reused worktree (fetch, clean tree, HEAD == `origin/<head>`); refuses a diverged local branch, a fork head, the repo's own default branch, and — with `EXPECT_SHA` — any HEAD that is not the recorded anchor. The `pr-fix` isolation half |
