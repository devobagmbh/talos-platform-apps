...
- **`required` vs `optional` in `customization.yaml`** (DR-0004, #794): `required.*` is the **must-supply** channel — without it the workload does not function. A key the artifact carries a **working baked default** for, which a consumer sets only to *change* behaviour (e.g. a `${INGESTER_REPLICATION_FACTOR:1}` placeholder resolved by `-config.expand-env=true`), goes under the additive top-level **`optional.env_keys`** — an object per key (`name`, `description`, `default`, optional descriptive `group`) — never under `required`. The two channels are disjoint and optional names are unique; `task validate:contract` enforces both over the real component files, since JSON Schema can express neither. Two edits to a shipped optional key are **breaking**, and both fail silently: renaming or removing it (env expansion falls back to the baked default, so the consumer loses their setting with no error) and **changing its `default`** (runtime behaviour changes for every consumer who left the key unset). Both need a major bump and a migration note.
