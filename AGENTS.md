...
`knowledge/` is the OKF bundle and **durable project memory** (DR-0002). The general agent-maintenance rules (treat the wiki as durable memory, keep docs in sync, validate before finishing) live in the `openknowledge rules`-managed block at the end of this file — regenerate it with `openknowledge rules apply docs --path knowledge --file AGENTS.md --target generic`. **This repo's specific freshness contract**, which that generic block does not encode: when a change touches a file listed in a `knowledge/**` concept's `sources:` frontmatter, re-verify that concept's present-tense claims and **bump its `timestamp:` in the same PR** (log substantive changes in `knowledge/log.md`). Surfaced mechanically by `task okf:freshness` — **advisory today** (run by `okf-freshness.yml`), blocking flip tracked in #541.
