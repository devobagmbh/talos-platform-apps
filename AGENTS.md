...
- **Sanctioned RBAC narrowing (optional, per component)**: a component MAY declare `rbac_policy` in `compatibility.yaml` to record that narrowing its cluster-wide manager `ClusterRoleBinding` to namespace scope is a **supported** consumer overlay (ADR-0024 names RBAC consumer-overlayable). `class: narrowable` obliges the component to carry a golden triple file (`rbac-baseline.triples`, the accepted rule set of the named `ClusterRole`) and a `### Namespace-scoped operation` README section naming the role plus every resource declared `inert_when_namespaced`; every resource the role names MUST be classified in `catalog/rbac-resource-scope.yaml`. Absence of the block means not narrowable and gates nothing. Gated by **`task validate:rbac-narrowing`** (in `task ci`, red-greened by `task test:rbac-narrowing`): because a narrowing consumer binds the shipped `ClusterRole` **by name**, an upstream chart bump would otherwise widen that consumer's grant — or falsify the documented cost — silently. Rationale + named limits: `knowledge/decisions/DR-0004-rbac-narrowing-overlay-sanction.md`.
