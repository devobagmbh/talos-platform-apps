...
- An agent that cannot regenerate it records the artifact as **not verifiable
  ...
  `not_locally_verifiable` entry naming the path; for a read-only reviewer it
  is a finding of the form `not-read: <path> absent` naming the unread
  artifact, and `verdict: needs-info` when its judgment depended on it.
