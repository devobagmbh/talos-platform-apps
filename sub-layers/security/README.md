...
Runtime security tooling for the consumer cluster — workload-level threat
detection and enforcement that runs *on* the cluster, distinct from the
secret-management tooling in `secrets`. The OCI distribution unit is the
component; this directory is the organisational bracket (ADR-0009).
