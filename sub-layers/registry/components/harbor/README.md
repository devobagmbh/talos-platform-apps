...
## Render profile (Tag 1: pull-through-cache consumer)
...
A consumer runs a single bundled Harbor (own Postgres + Redis + registry-blob PVCs). Where several clusters share one registry, only the registry-hosting cluster deploys this component and the others pull through it (ADR-0026).
