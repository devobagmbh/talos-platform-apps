...
CDI publishes no anonymously-pullable Helm
...
This workload ships the `CDI` CR as a **catalog default**, taken from the
`talos-platform-base` migration — **not** from upstream's `cdi-cr.yaml`. It diverges
from that upstream CR in three ways, all inherited from the migration and none of them
an upstream default. It omits `config.featureGates: [HonorWaitForFirstConsumer]`, which
upstream's released `cdi-cr.yaml` enables
([waitforfirstconsumer-storage-handling](https://github.com/kubevirt/containerized-data-importer/blob/main/doc/waitforfirstconsumer-storage-handling.md)):
without the gate CDI schedules its worker pod immediately and binds the PVC on **that**
pod's node, so on a `WaitForFirstConsumer` StorageClass the disk can land on a node the
VM is not scheduled to. It also omits the `infra`/`workload` `kubernetes.io/os: linux`
nodeSelectors and adds `app.kubernetes.io/*` labels.
