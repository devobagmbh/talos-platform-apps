...
- **Shape (a)** — `ConfigMap` `mimir-runtime-config` (non-secret), `envFrom`:
  `S3_ENDPOINT`, `S3_REGION`, `S3_BUCKET_BLOCKS`, `S3_BUCKET_RULER`.
...
- **`mimir-runtime-config` `ConfigMap`** with keys `S3_ENDPOINT` (the explicit Garage S3
  ...
  `garage`), `S3_BUCKET_BLOCKS`, `S3_BUCKET_RULER`.
...
addressing, TLS on (`insecure: false`). Only the connection *values* are
consumer-supplied.
