...
- **`/ship-catalog-app <app>`** (`.claude/skills/ship-catalog-app/`) — **End-to-End-Orchestrator** für den vollen plan→approve→build-Bogen EINER Catalog-App in einer Session. Reine Orchestrierungsschicht: ruft die zwei Skills unten auf, dupliziert ihre Logik/Conventions NICHT. Drei Invarianten: (1) **verpflichtender menschlicher Plan-Freigabe-Gate** zwischen Plan und N PRs (AskUserQuestion; headless → stop-after-plan); (2) **Merge-Gate mechanisch vorklassifiziert, Build-Skill als Backstop** — abhängige Komponenten brauchen den Merge ihrer Dependency, bevor sie baubar sind (jeder Build läuft im frischen Worktree von `origin/main`; `build-catalog-component`s eigener Dependency-Existence-Check verlangt `external_dependencies` UND Build-Zeit-`requires:` im Tree). Ship klassifiziert den Merge-Gate über einen **billigen, konservativen Pre-Check**: eine Komponente, deren plan-deklarierte `external_dependencies` nicht alle nach `main` gemerged sind, ist `awaiting-merge` und wird **nicht** attemptet (kein verschwendeter Dispatch). Der Pre-Check ist strikt schwächer als der Build-Gate (`external_dependencies ⊆ external_dependencies ∪ requires:`), kann also nie etwas fälschlich zulassen — der Build-Skill-Check bleibt autoritativer Backstop für Build-Zeit-`requires:`. Damit ist der Stop-Grund eindeutig: jeder Stop NACH einem Attempt ist `build-incomplete`, kein Merge-Gate; (3) **re-run-resumierbar** — pro Komponente wird der Git-Status (`done` auf main / `in-flight` als offener PR-Branch / pending) just-in-time gelesen, kein Ship-State-File. Positiver Completion-Verify (`git ls-remote` + `gh pr list`) vor jedem Vorrücken — „done" wird nie inferiert. Kein neues CONVENTIONS.md — DRY-Quelle sind die zwei Sub-Skills.
