...
- **`/pr-fix <N>`** (`.claude/skills/pr-fix/`) — **review-response** half of the PR family (`pr-gate` reviews, `pr-fix` responds, `pr-enqueue` merges). Harvests every review finding and unresolved thread into an append-only ledger, triages each against the PR head (closed vocabulary: `fix` / `rejected-with-evidence` / `deferred-to-issue` / `ask-operator`), fixes the survivors in `.claude/worktrees/pr-<N>` one commit per finding, and stops at the push boundary. Deterministic halves live in the Taskfile (`pr:fix:facts`, `pr:fix:admit` + `test:pr-fix-admit` in `task ci`, `worktree:pr`), so the admissibility decision is red-green bound rather than prose. Spec/DRY source: the SKILL.md.
